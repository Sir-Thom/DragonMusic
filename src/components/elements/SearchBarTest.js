...
export default  function withSearch(WrappedComponent, searchFunction){
  return class extends React.Component {		// On retourne un nouveau composant
    ...
    return(
        ...
        <button onClick={() => this.modifyValue()} className=" inline-block ms-auto px-6 transition ease-in hover:scale-200 py-2 border-2 border-violet-600 text-violet-600 font-medium text-xs leading-tight uppercase rounded-full hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 " 
